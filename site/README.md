...
### Known devnet limitation

On devnet, individual transactions tagged with `Root-TX` are not reliably propagated to the CDN, so `gateway.irys.xyz/mutable/<rootId>/` may return 404 even though the transaction was accepted. The folder chain and the redirect chain both build correctly; the failure is at the CDN indexing layer. The same pipeline works as expected on mainnet. Until devnet stabilizes, use the per-deploy manifest URL (`gateway.irys.xyz/<manifestId>/`) as the public link and treat `/mutable/` as the update mechanism only.
