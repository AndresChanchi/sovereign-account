...
The `crates/` directory contains the two Rust crates that are published to
crates.io. They are the materialization of the formal model as reusable
libraries.

| Crate | Role | Published as |
|---|---|---|
| `kipio_dafny_runtime` | Runtime support for generated code | `kipio_dafny_runtime` |
| `kipio_account_generated` | Translated Kipio Account domain model | `kipio_account_generated` |

Both are generated by [tools/export_rust_production.py](https://github.com/AndresChanchi/vaultchain-sovereign-agent/tree/develop/domain/account/formal). from the Dafny
tree. Neither is edited by hand. Consumers depend on
`kipio_account_generated` directly, or on a harness that wraps it for a
specific chain.

See [`crates/kipio_dafny_runtime/README.md`](crates/kipio_dafny_runtime/README.md)
and [`crates/kipio_account_generated/README.md`](crates/kipio_account_generated/README.md)
for the crate-level details.
